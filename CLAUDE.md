...
- **AppViewModel**: `@Observable` 全局状态容器，持有 Services 和 ViewModels，通过 `.environment()` 注入
...
Chat 模块采用 SessionService + SessionHandle 两层架构。View 直接读 handle，不经过中间 ViewModel。

### 核心实体

| 实体 | 类型 | 数量 | 职责 |
|------|------|------|------|
| SessionService | @Observable | 1 | handle 工厂 + session 生命周期 + active session 管理 + 消息提交路由 |
| SessionHandle | @Observable | N（per-session） | session 全部状态 + 命令 + pre-launch config + draft + UI hints |
| ChatRendererService | NSObject | 1 | 全局 WKWebView + Bridge（聊天渲染） |
| PlanRendererService | NSObject | 1 | 全局 Plan WebView（plan 全屏阅读） |
| CompletionViewModel | @Observable | N（per-view） | 补全列表状态管理（InputBarView @State 持有） |
...
│   ├── handles: [String: SessionHandle] ×N（唯一 per-session 缓存）
│   ├── activeSessionId: String（当前活跃 session）
│   └── activeHandle: SessionHandle?（computed）
├── chatRendererService: ChatRendererService
│   └── bridge: WebViewBridge
├── planRendererService: PlanRendererService
├── sidebarViewModel: SidebarViewModel
│   └── 依赖: sessionService（观察 allHandles，自驱动 rebuild）
└── permissionCardTypes: [String: PermissionCardType]（per-permission card VM 缓存）

SessionHandle（per-session 唯一 source of truth）
├── status: .notStarted | .inactive | .idle | .responding | .interrupting | .starting
├── config: originPath, selectedModel, selectedEffort, isWorktree, permissionMode, ...
├── runtime: cwd, contextUsedTokens, pendingPermissions, queuedMessages, slashCommands
├── draft: draftText（debounced UserDefaults 持久化）
├── UI hints: activePlanReviewId, planCommentText, pendingCommentSelections, isAtBottom, processExitError
└── branchMonitor: GitBranchMonitor
```

### 数据流

- **状态渲染**：SessionHandle 属性变化 → SwiftUI 自动追踪 → View 重渲染。无手动 observation，无副本
- **消息推送**：所有 handle（包括后台）实时推送消息到 bridge → React 按 conversationId 分存
- **Session 切换**：SidebarView onSelect → sessionService.activateSession(id) → 设 activeSessionId（didSet 自动切 bridge）。选中状态 source of truth 是 `sessionService.activeSessionId`
- **消息提交**：InputBarView → sessionService.submitMessage(handle, text) → 路由: notStarted→launch, inactive→resume, idle→send

### UI 层胶水

RootView 是唯一同时知道 SidebarViewModel 和 SessionService 的地方。SidebarView 通过 `selection` Binding 回调 SessionService。ChatView 和 InputBarView 通过 `@Environment(AppViewModel.self)` 读 service。

### 规范

- **View 读两种东西**：service（handle 等）和自己的 @State。不经过中间 ViewModel 代理
- **SessionHandle 是 per-session 唯一 source of truth**。无副本，无双源属性
- **activeSessionId 在 SessionService 上**：所有 session 切换收口在 service 层
- 禁止在 View 中直接修改 session 的 processing/interrupting 状态，必须通过 SessionHandle 方法触发
- UI 层只读取状态，不维护独立的 session 状态
- InputBar 的操作（interrupt、selectModel 等）直调 `handle.method()`
- 需要跨 session 协调的操作通过 `appVM.handleRouterAction()` 或 `sessionService` 方法
- 新增 session 运行时状态：在 SessionHandle 加 `@Observable` 属性，View 通过 handle 直读

## Session 运行时状态

`SessionHandle`（`@Observable`，定义在 `Services/Session/SessionHandle.swift`）是单个会话的运行时句柄，持有可观察状态、pre-launch 配置、draft、UI hints 和交互命令。

`SessionService`（定义在 `Services/Session/SessionService.swift`）管理 SessionHandle 的生命周期（创建/缓存/start/stop/remove），持有 activeSessionId，提供消息提交路由。
...
| **Event**（`SessionEvent` + `AsyncStream`） | 离散事件 → 触发副作用 | SessionHandle 内部自处理 | statusChanged、permissionsChanged、processExited |

**判断标准：** 如果消费者需要「当前值是什么」→ Observe；如果消费者需要「刚刚发生了什么」→ Event。

**规范：**
- 禁止在 View 中缓存 SessionHandle 属性副本作为状态来源，通过 handle 属性直读
- 本地操作（send/interrupt/setPermissionMode）只写 stdin 请求 CLI 变更，不直接改 observable 值
- 新增运行时状态字段：在 SessionHandle 加 `@Observable` 属性，View 直读
- SessionHandle 内部自处理事件副作用（status→clearDraft, cwd→updateBranchMonitor, processExit→setError）
- 新增事件类型：在 `SessionEvent` 加 case → SessionHandle 相应位置 `emit()` + 内部 handler
...
**规范：**
- category 使用类名（不含模块前缀），如 `"SessionHandle"`、`"SessionService"`
...
│   ├── ccterm.xcodeproj/
│   ├── ccterm/               # 应用源码
│   │   ├── App/              # CCTermApp, AppViewModel, RootView
...
- **Git submodules**：首次构建时 `build.sh` 会自动初始化（fzf），也可手动 `git submodule update --init --recursive`
