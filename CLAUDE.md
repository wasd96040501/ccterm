...
Runtime shape of the layer table is one loop: **Service/Store** (single source of truth) → data down → **Controller** (subscribes, translates state into view input) → data down → **View** (renders only what it's handed) → event up (target-action / delegate / closure) → **Controller** (translates event into a Service call) → **Service** mutates → next data-down cycle.

- **State lives in exactly one source of truth.** The View caches no business truth; the Controller invents no truth (only subscribes to a Service). This is a "quasi"-unidirectional flow held by discipline, not a reducer framework — don't turn every click into an action object.
- **Never let a View mutate a Model directly, or hold+call a Service directly.** That makes a two-way / cyclic dependency and a second source of truth. A View reports intent via closure/delegate to its Controller; the Controller calls the injected Service.
...
5. **Assembly + lifecycle → Controller (+ Coordinator for navigation).** The VC subscribes the Service, feeds the View, translates the View's events into Service calls. "Which screen next / who creates it" goes to a Coordinator (see below), not the VC.
...
| KVO | 1:many | no | observe a *system* object's property | `NSKeyValueObservation` alive = observing |
...
- **Diffable data source for dynamic data.** Use `NSTableViewDiffableDataSource`; apply a snapshot instead of hand-tracking `insert/remove/move` (the classic source of `NSInternalInconsistencyException` and lost selection). **The item identifier must be *identity*** (a stable `id: String`/`UUID`), never the whole mutable model value — if its `hashValue` changes with content (e.g. read state), the framework treats an edited row as delete+insert (wrong animation, lost selection). Express a content change via `reloadItems([id])` (macOS 11+) or the cheaper `reconfigureItems([id])` (macOS 13+); both keep the item set unchanged, so no insert/delete animation and no lost selection. Prefer `animatingDifferences: false` for content-only updates.
...
- **UI is main-thread only; `@MainActor` makes that a compile-time check.** Every `NSView`/`NSViewController` subclass and any type that touches UI is `@MainActor`. Do the expensive pure-compute/IO off the main actor (`await` a non-isolated async function; a non-detached `Task {}` started from a `@MainActor` method inherits main-actor isolation and only truly leaves on `await`, returning to main automatically). Never write a UI property off-main, and never block the main RunLoop (sync network, `Thread.sleep`, semaphore `wait`, `DispatchQueue.global().sync`) — that freezes the UI. `@MainActor` types keep a `nonisolated deinit`.
