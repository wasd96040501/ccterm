...
- **Waiting for a PR**: never sleep-and-poll `gh pr checks` / `gh pr view` in the foreground. Use `scripts/wait-for-pr.sh <pr#>` with `run_in_background: true` — it blocks until a real terminal state (`READY`, `CHECKS_FAILED`, `CONFLICT`, `REVIEW_CHANGES_REQUESTED`, `MERGED`, `CLOSED`, `TIMEOUT`, `NO_CHECKS`) and the harness will notify on completion. Foreground polling burns prompt cache (>5min loses warmth) and loses state when the conversation is compressed.
