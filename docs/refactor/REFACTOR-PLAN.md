...
> 本文是对 CCTerm 整个组件树与数据流的重构方案。结论由一轮并行多代理调研（12 个子系统调研 → 横切分析 → 4 组「设计 + 对抗验证」）汇总而成；每条设计结论都经过**独立对抗审核员**针对真实代码的挑战，审核修正已折叠进正文。各节点的原始产物落盘在 [`docs/refactor/nodes/`](nodes/)，索引见 §13。
>
...
6. **host-sizing 纪律** —— 整 pane `[]`+四边钉；component `[.intrinsicContentSize]`+位置钉。聊天栏宿主的 `[.intrinsicContentSize]` 不对称**保留**，**不**折进 fill-pane helper。
