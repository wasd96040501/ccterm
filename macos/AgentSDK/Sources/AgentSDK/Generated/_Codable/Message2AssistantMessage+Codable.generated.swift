...
extension Message2AssistantMessage {
    public init(json: Any) throws {
        let r = try JSONReader(json, context: "Message2AssistantMessage")
        self._raw = r.dict
        self.container = r.raw("container")
        self.content = try? r.decodeArrayIfPresent("content")
        self.contextManagement = r.decodeIfPresent("context_management")
        self.id = r.string("id")
        self.model = r.string("model")
        self.role = r.string("role")
        self.stopReason = r.string("stop_reason")
        self.stopSequence = r.string("stop_sequence")
...
extension Message2AssistantMessage {
    public func toTypedJSON() -> Any {
        var d: [String: Any] = [:]
        if let v = container { d["container"] = v }
        if let v = content { d["content"] = v.map { $0.toTypedJSON() } }
        if let v = contextManagement { d["context_management"] = v.toTypedJSON() }
        if let v = id { d["id"] = v }
