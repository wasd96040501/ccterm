...
struct RootView2: View {
    ...
    private var detailContentReleaseBranches: some View {
        if selectedSessionId == SidebarView2.archiveTag {
            ...
            // Chrome (overlays, composeStack, input bar) follows it
            // immediately. The transcript itself is owned by
            // `TranscriptSwapHost`, which manages the bake double-buffer
            // and the live-retainer / ephemeral-remount split internally
            // — the on-screen view only updates once the incoming
            // session's controller flips `firstScreenReady`.
            TranscriptSwapHost(targetSessionId: sid)
