...
struct RootView2: View {
    ...
    /// The sessionId actually bound to `ChatHistoryView`. Mirrors
    /// `effectiveSessionId` immediately on a switch — the *visible*
    /// stability across the swap comes from `ViewTransitionContainer`
    /// baking a snapshot of the previous view's pixels until the new
    /// session's controller signals `firstScreenAnchored`. See
    /// `syncVisibleSession(to:)`.
    @State private var visibleSessionId: String?
    /// Drives the chat-area double-buffer overlay. Captures the
    /// current transcript pixels on a sidebar switch and holds them on
    /// top until the target session anchors its first screen. See
    /// `ViewTransitionContainer` for the mechanism.
    @State private var chatTransition = ViewTransitionController()
    ...
    private var detailContent: some View {
        if selectedSessionId == SidebarView2.transcriptDemoTag {
            ...
            // immediately. The transcript itself is wrapped in a
            // `ViewTransitionContainer` whose overlay holds the
            // previous view's pixels while the new ChatHistoryView's
            // controller anchors its first screen. The `?? sid`
            // fallback covers the initial frame (visibleSession still
            // nil) so we never render an empty detail.
            //
            // `.id(visibleSid)` pins ChatHistoryView identity across
            // the NewSession → History transition: the draft UUID
            // becomes the history sessionId after the first send, so
            // SwiftUI doesn't rebuild the NSView. The bottom
            // InputBarView2 plays both the "draft launcher" and
            // "history continuation" roles — its onSubmit closure
            // branches on `handle.hasRecord` to trigger the
            // first-start side effects only when needed.
            //
            // The transition container wraps the chat *transcript*
            // only — chrome (fade scrims, compose stack, input bar)
            // sits outside so it animates through its own existing
            // transitions while the transcript area is statically
            // covered by the baked overlay.
            ...
            ViewTransitionContainer(controller: chatTransition) {
                ...
                // Reactive double-buffered swap. The modifier owns the
                // `effectiveSessionId → visibleSessionId` transition,
                // bakes the previous view's pixels into `chatTransition`,
                // and uses `.onChange` on the visible controller's
                // `firstScreenAnchored` to release the overlay the
                // moment the new table anchors. No polling.
                .chatSwap(
                    target: effectiveSessionId,
