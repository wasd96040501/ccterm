...
| `ChatSessionViewController` | NSViewController | 1 | Owns the `Transcript2ScrollView` directly via `TranscriptScrollViewFactory`; mounts AppKit `TranscriptScrimView` / `TranscriptBottomScrimView` for top/bottom fades (hitTest passthrough so the table below sees clicks + cursor rects), and hosts the SwiftUI chat resting bar via `NSHostingView` (bottom-anchored to the bar's height — **always**, never full-bleed). Does **not** observe `MainSelectionModel`; the router drives session attach (`loadHistory`, `setLoading`) imperatively via `present(sessionId:)`. Mounted only for `.session(_)` / `.none`. |
