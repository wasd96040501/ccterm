...
| `MainSplitViewController` | NSSplitViewController | 1 | Sidebar item (hosts `SidebarViewController`) + detail item (`TranscriptDetailViewController`). |
...
| `SidebarViewController` | NSViewController (AppKit) | 1 | Reads `SessionManager.records` to render the `NSOutlineView` history list; writes back the selected `sessionId` to `MainSelectionModel.selectedSessionId` via the outline-view selection bridge. |
...
└── mainWindowController: MainWindowController
    ...
    └── MainSplitViewController
        ├── Sidebar item → SidebarViewController (AppKit NSOutlineView)
...
- **Session switch** — `SidebarViewController` writes a new value → `MainSelectionModel.selectedSessionId` changes → `TranscriptDetailViewController`'s observation tracking fires → `attachSession(_:)` tears down the old `Transcript2ScrollView` (via `TranscriptScrollViewFactory.dismantle`) and constructs a fresh one for the new session's controller. The factory binds the new `NSTableView` to `session.controller.coordinator` and runs `noteNumberOfRowsChanged` so the table picks up whatever block state accumulated while detached, in source phase — before the host's `addSubview` / `layoutSubtreeIfNeeded` / `scrollToTail` sequence runs.
