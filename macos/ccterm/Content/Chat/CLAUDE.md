...
| `ChatSessionViewController` | NSViewController | 1 | Owns the `Transcript2ScrollView` directly via `TranscriptScrollViewFactory`; mounts the AppKit `TranscriptTopScrimView` top fade (hitTest passthrough for the transcript, but its band doubles as the title-bar drag/zoom region), and hosts the **merged bottom cluster** (`ChatBottomCluster` — fade + chat resting bar + floating permission card in one SwiftUI tree) via a single full-width, bottom-anchored `NSHostingView<ChatBottomClusterRoot>` (`bottomClusterHost`, `sizingOptions = [.intrinsicContentSize]`). The host's top edge floats at `view.bottom − cluster height`, so the transcript above stays uncovered and keeps its clicks — non-occluding by geometry, no hit-test trick (the deleted `PassthroughHostingView`). Does **not** observe `MainSelectionModel`; the router drives session attach (`loadHistory`, `setLoading`) imperatively via `present(sessionId:)`. Mounted only for `.session(_)` / `.none`. |
...
└── mainWindowController: MainWindowController
    ...
    └── MainSplitViewController
        ...
        └── Detail item → DetailRouterViewController   ← one child VC per selection
            ...
            │   ├── topScrim: TranscriptTopScrimView (AppKit, hitTest passthrough; title-bar band)
            │   └── bottomClusterHost: NSHostingView<ChatBottomClusterRoot>  (full-width, bottom-anchored; fade + resting bar + permission card in one SwiftUI tree)
...
- Cross-view coordination uses closures injected from `ChatSessionViewController` (e.g. `onSubmit`, `onBuiltinCommand`). Don't introduce a new ViewModel layer.
