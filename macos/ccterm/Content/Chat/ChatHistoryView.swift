...
enum ChatHistoryRenderCase: Equatable {
    ...
    /// `NativeTranscriptView(entries: handle.messages)`。没有 ProgressView ——
    /// empty entries 和已加载空聊天视觉等价。
    case transcript
...
///
/// 无 ProgressView —— 两段式 loadHistory 的 tail 一般 < 50 ms，spinner 闪一下
/// 反而劣化视觉。非 `.failed` 的所有状态都直渲 `NativeTranscriptView`，entries
/// 空时视觉上等价「空聊天」。
struct ChatHistoryView: View {
    ...
    /// TranscriptController 在首次 Phase 1 merge 完成时 emit OpenMetrics。
