...
struct ChatHistoryView: View {
    ...
    var body: some View {
        ...
        .task(id: sessionId) {
            ...
            // (re-)attach is owned by the coordinator/controller
            // handshake (`onTableAttached` → `pendingInitial` →
            // `onLayoutReady`), not by this view — at `.task` time
            // SwiftUI may not have committed the new
            // `NativeTranscript2View` yet and any scroll attempt here
            // would race the NSView install.
