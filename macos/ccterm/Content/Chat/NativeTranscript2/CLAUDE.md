...
Only this `noteHeightOfRows` path needs the flush. The `insertRows` paths (`.prepend` / `.append` / `.insert`) settle new-row geometry synchronously inside `endUpdates` (no estimated heights — §2.1), so their `.saveVisible` anchor already reads real `rect(ofRow:)` with no forced tile. See REFACTOR-PLAN §5.1 for the asymmetry.
