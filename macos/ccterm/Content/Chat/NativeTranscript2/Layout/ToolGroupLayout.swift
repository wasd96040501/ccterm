...
struct ToolGroupLayout: @unchecked Sendable {
    ...
    /// Folded children drop out at region-build time (their `body` is
    /// nil) — `Coordinator.expandForSearchHit` covers the inverse case
    /// where a user folds a child *after* searching: the stale hit's
    /// range still encodes the childIndex, nav re-expands the child,
    /// and the new layout's region picks the highlight back up.
    var selectionAdapter: SelectionAdapter? {
        var regions: [ToolChildSelectionRegions.Region] = []
        for (idx, entry) in items.enumerated() {
            guard let body = entry.body else { continue }
            regions.append(
                contentsOf: ToolChildSelectionRegions.regions(childIndex: idx, body: body))
        }
        return ToolChildSelectionRegions.adapter(regions: regions)
    }
