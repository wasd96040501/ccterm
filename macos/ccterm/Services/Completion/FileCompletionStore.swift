...
final class FileCompletionStore {
    ...
    /// Pre-warm the cache for a directory. Enqueues `ensureLoaded` on the
    /// serial queue so any subsequent `complete(...)` call naturally lands
    /// behind the warm; if the warm has finished, the cache hit makes the
    /// complete call instant. Idempotent and cheap to call on every view
    /// mount / config change. Mirrors the `GitProbe.loadHeavy` shape used
    /// by the branch picker.
    func warm(directory: String) {
        queue.async { [weak self] in
            _ = self?.ensureLoaded(directory: directory)
        }
    }

    func warm(directories: [String]) {
