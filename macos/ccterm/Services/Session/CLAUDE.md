...
2. **Switch-away → switch-back is O(1) on the renderer side.** No JSONL re-read, no markdown reparse, no block-list rebuild. The new `NSTableView` rebinds to the same coordinator on mount; `TranscriptScrollViewFactory.make` calls `noteNumberOfRowsChanged` immediately after binding the dataSource so the table picks up the coordinator's current `blocks` synchronously, in source phase.
