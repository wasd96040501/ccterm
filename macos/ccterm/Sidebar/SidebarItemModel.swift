...
/// Heterogeneous rows the sidebar's `NSOutlineView` walks. Kept as a
/// reference type so identity-based row reuse (`NSOutlineView` keys
/// internal state on `===`) stays stable across `reloadData()` calls.
final class SidebarItemNode {
    enum Kind {
        /// Fixed top-of-sidebar tab (New Session, Archive, DEBUG demos).
        case fixed(FixedKind)
        /// A folder header — toggleable parent that contains history rows.
    ...
    /// History children for folder nodes. Empty otherwise.
...
enum FixedKind: CaseIterable {
    ...
    /// English title for the row label. Wrapped in `String(localized:)`
    /// at construction time so the catalog lookup runs in the view layer.
    var title: String {
        switch self {
