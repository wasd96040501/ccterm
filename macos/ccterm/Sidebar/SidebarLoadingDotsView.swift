...
/// AppKit port of the SwiftUI `SidebarLoadingDots`: three small dots
/// whose opacities cycle in a left-to-right wave. The frame timer is
/// `NSView.displayLink(target:selector:)` so the cadence matches the
/// host display; opacities are sampled from the absolute wall-clock
/// (`Date.timeIntervalSinceReferenceDate`), so multiple instances of
/// the view — including the transcript's pill — breathe in lockstep.
///
