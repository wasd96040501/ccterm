...
| Running pill trailing chip — live turn clock + `↑in ↓out` counter (`{elapsed} · ↑in ↓out` in the `LoadingPillUsageView` subview) ([source](../ccterm/Content/Chat/NativeTranscript2/AppKit/LoadingPillUsageView.swift)) | `LoadingPillUsageSnapshotTests` ([source](LoadingPillUsageSnapshotTests.swift)) | `/tmp/ccterm-screenshots/LoadingPillUsage.png` |
